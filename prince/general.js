...
// ðŸ”  Small caps formatter (like Gifted)
...
gmd({
  ...
  try {
    ...
    await reply(`âŒ An error occurred while processing the message.`);
  }
});
...
gmd({ 
  ...
  description: "check bot uptime status.",
    ...
    await Prince.sendMessage(from, {
      text: `â±ï¸ Uptime: ${days}d ${hours}h ${minutes}m ${seconds}s`,
      contextInfo: getContextInfo(null, newsletterJid, botName)
    }, { quoted: mek });
      await react("âœ…");
  }
...
gmd({
  ...
  if (!isSuperUser) {
    return reply(`âŒ Owner Only Command!`);
  }
  ...
  try {
    ...
    if (quotedMsg.imageMessage) {
      ...
      return reply(`âŒ Unsupported message type.`);
    }

    await Prince.sendMessage(sender, mediaData, { quoted: mek });
    // await reply(`âœ… Saved Successfully!`);
    await react("âœ…");

  } catch (error) {
    console.error("Save Error:", error);
    await reply(`âŒ Failed to save the message. Error: ${error.message}`);
  }
});
